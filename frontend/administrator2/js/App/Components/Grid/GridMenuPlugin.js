 ...
 */
Ext.define("Administrator.Components.Grid.GridMenuPlugin", {
	...
	init: function(grid) {
		...
		this.menu = new Ext.menu.Menu({
			...
			items: [{
				...
				menu: [{
					icon: Administrator.getResourcePath() + 'resources/mimetypes/csv.png',
					text: 'Export as CSV (.csv)',
					handler: this.exportCSV,
					scope: this
				},{
					icon: Administrator.getResourcePath() + 'resources/fugue-icons/icons/blue-document-excel.png',
					text: 'Export as Excel XML (.xlsx)',
					handler: this.exportXLSX,
					scope: this
				},{
					icon: Administrator.getResourcePath() + 'resources/icons/mediawiki_icon.png',
